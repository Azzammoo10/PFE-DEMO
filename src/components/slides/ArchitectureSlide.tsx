...
import Volet2VecteursDetectionTable from './Volet2VecteursDetectionTable';
import Volet3PentestAnimation from './Volet3PentestAnimation';
import Volet5WazuhSocSchema from './Volet5WazuhSocSchema';
...
export default function ArchitectureSlide({ n }: SlideProps) {
  ...
  const getSubTitle = () => {
    if (subStep === 1) return 'VOLET 1/5 : Infrastructure & Réseau Virtualisé (4 VMs)';
    if (subStep === 2) return 'VOLET 2/5 : DLP Manager & Micro-Agents Python';
    if (subStep === 3) return 'VOLET 3/5 : Les 5 Vecteurs de Détection (Agent Endpoint)';
    if (subStep === 4) return 'VOLET 4/5 : Conception de la Validation Offensive (Pentest)';
    return 'VOLET 5/5 : Supervision SOC Wazuh & Active Response';
  };
      ...
      <div className="w-full h-full flex flex-col items-center justify-between relative overflow-hidden p-0">
        ...
        <div style={{
          ...
          <div style={{
            ...
            {subStep === 1 && <Network style={{ width: '1vw', height: '1vw', color: '#0b66d5' }} />}
            {subStep === 2 && <Cpu style={{ width: '1vw', height: '1vw', color: '#0b66d5' }} />}
            {subStep === 3 && <ShieldCheck style={{ width: '1vw', height: '1vw', color: '#0b66d5' }} />}
            {subStep === 4 && <Flame style={{ width: '1vw', height: '1vw', color: '#dc2626' }} />}
            {subStep === 5 && <ShieldAlert style={{ width: '1vw', height: '1vw', color: '#00008f' }} />}
            <span style={{ color: subStep === 4 ? '#b91c1c' : (subStep === 5 ? '#00008f' : '#0b66d5') }}>
              {getSubTitle()}
            </span>
          </div>
        </div>

        {/* Main Stage Display (Maximized Height for all 5 Volets) */}
        <div style={{
          position: 'relative',
          width: '100%',
          ...
          <AnimatePresence mode="wait" custom={slideDirection}>
            ...
            {/* VOLET 2.1: Les 5 Vecteurs de Détection (Agent Endpoint) */}
            {subStep === 3 && (
              <motion.div
                key="sub-3"
                custom={slideDirection}
